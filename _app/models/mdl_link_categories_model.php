...
class Mdl_link_categories_model extends MY_Model {

	const DB_TABLE = 'mdl_link_categories';
...
FROM mdl_link_categories c1  
LEFT OUTER JOIN mdl_link_categories c2 on c1.parent_id = c2.id 
 ...
 * /
	 ...
	 */
	public function get_mdl_link_categories_with_parent_name($order_by = "name") {
		...
		if ($this->session->userdata('member_id') != '') {
			...
			$this->db->from('mdl_link_categories AS c1');
			$this->db->join('mdl_link_categories AS c2', 'c2.id = c1.parent_id', 'left outer');
	...
	} // end of - function get_mdl_link_categories_with_parent_name


	/**
 	 * Get categories from the DB and return them as an array.
	 */
	public function get_mdl_link_categories($order_by = "name") {
		
		// If a member isn't signed in then return an empty array.		
		if ($this->session->userdata('member_id') != '') {
		
			$this->db->from('mdl_link_categories');
	...
	} // end of - function get_mdl_link_categories
	...
	/**
 	 * Get categories from the DB and return them as an array.
	 */
	public function get_mdl_link_categories_with_no_parent($order_by = "name") {
	...
	} // end of - function get_mdl_link_categories_with_no_parent	
	...
	/**
 	 * Get categories from the DB and return them as an array.
	 */
	public function get_mdl_link_categories_with_p($order_by = "name") {
	...
	} // end of - function get_mdl_link_categories_with_p
	
		
	/**
 	 * Get categories from the DB and return them as an array.
	 */
	public function get_mdl_link_categories_no_p($order_by = "name") {
	...
	} // end of - function get_mdl_link_categories_no_p		
	...
	public function get_category_sub_category_object_array() {
		...
		$a_categories_no_p = $this->get_mdl_link_categories_no_p();
		$a_categories_w_p = array();
		$a_categories_w_p = $this->get_mdl_link_categories_with_p();				
	 ...
	 */
	public function get_mdl_link_categories_with_number_of_records() {

		$this->db->select('lc.id, lc.name, count(l.id) AS num_links');
		$this->db->from('mdl_link_categories AS lc');
		$this->db->join('mdl_links AS l', 'lc.id = l.category_id', 'left');
		$this->db->where('lc.display', 'yes');
		//$this->db->where('r.display', 'yes');
		$this->db->group_by('lc.id');
		$this->db->order_by('lc.name', 'asc'); 
		$query = $this->db->get();
	...
	} // end of - function get_mdl_link_categories_with_number_of_records	
...
/* End of file mdl_link_categories_model.php */
/* Location: ./application/models/mdl_link_categories_model.php */