	...
	echo '<li class="list-group-item thin ' . $background_color . ' clearfix">';
		...
		echo '<div class="action-link-holder">' . chr(10);											
			...
			if ($this->session->userdata('member_id') == $link['member_id']) {
				echo '<a href="' . site_url("mdl-links/view/") . '/' . $link['id'] . '" data-toggle="tooltip" title="Link Details"><span class="glyphicon glyphicon-eye-open action-link">&nbsp;</span></a>' . chr(10);
				echo '<a href="' . site_url("mdl-links/edit/") . '/' . $link['id'] . '" data-toggle="tooltip" title="Edit Link"><span class="glyphicon glyphicon-pencil action-link">&nbsp;</span></a>' . chr(10);
				echo '<a href="' . site_url("mdl-links/delete/") . '/' . $link['id'] . '" data-toggle="tooltip" title="Delete Link" onclick="return confirm(\'Are you sure you want to delete this link?\');"><span class="glyphicon glyphicon-remove action-link">&nbsp;</span></a>' . chr(10);
			}
		echo '</div>' . chr(10);
